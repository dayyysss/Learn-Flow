                    ...
                    <div class="p-0 mt-5 mb-5">
                            <div class="flex flex-col flex-center-between md:flex-row md:items-center md:justify-between center-between gap-2">
                                <div class="flex flex-wrap items-center gap-1">
                                    <button class="btn b-light btn-primary-light dk-theme-card-square"
                                        onclick="window.location.href='{{ route('artikel.create') }}'">
                                ...
                                <div class="w-full md:w-auto flex items-center gap-3">
                                    ...
                                    <button type="button"
                                        ...
                                        onclick="window.location='{{ route('artikel.index') }}'">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-dark-border-three">
                                    @foreach ($artikel as $item)
                                        <tr>
                                            ...
                                            <td class="p-6 py-4">{{ Str::limit($item->judul, 20, '...') }}</td>
                        ...
                        <div class="flex-center-between mt-5">
                            <div class="font-spline_sans text-sm text-gray-900 dark:text-dark-text">
                                Showing {{ $artikel->firstItem() }} to {{ $artikel->lastItem() }} of {{ $artikel->total() }} entries
                            </div>
                            <nav>
                                <ul class="flex items-center gap-1">
                                    ...
                                    @foreach ($artikel->getUrlRange(1, $artikel->lastPage()) as $page => $url)
                                        <li>
                                            <a href="{{ $url }}"
                                                class="font-spline_sans font-medium flex-center size-8 rounded-50 text-gray-900 dark:text-dark-text {{ $page == $artikel->currentPage() ? 'bg-primary-500 text-white' : '' }}">
                                                {{ $page }}
                                            </a>
                                        </li>
                                    @endforeach
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        //jika tombol ok di klik, kembali ke artikel sebelumnya
    ...
    // Menghandle checkbox
       ...
       function performAction(url, ids) {
           ...
           fetch(url, {
               ...
               if (response.ok) {
                   location.reload(); // Refresh artikel setelah berhasil
               } else {
